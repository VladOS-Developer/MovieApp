#🎬 MovieApp — каталог фильмов и сериалов с трейлерами

##MovieApp — iOS-приложение на Swift / UIKit для просмотра фильмов и сериалов, их трейлеров и актёров, с возможностью поиска и добавления в избранное. Использует TMDb API.

#🔗 TMDb Disclaimer
##This product uses the TMDB API but is not endorsed or certified by TMDB.

#🚀 Основные возможности

- Популярные и трендовые фильмы и сериалы

- Детальные страницы фильмов, сериалов и актёров

- Поиск по контенту

- Трейлеры через YouTubeiOSPlayerHelper

- Фильтрация по жанрам

- Избранное

- Passcode экран с безопасным хранением через Keychain

#🧩 Архитектура

*MVP (Model–View–Presenter)

- Protocol-Oriented Programming

- Dependency Injection через инициализаторы

- Разделение слоёв: DTO → Domain → ViewModel

#🌐 Работа с сетью

- TMDb API (фильмы, сериалы, актёры, трейлеры)

- Асинхронные запросы через async/await

- Репозитории + маппинг моделей

#💾 Работа с данными

- CoreData — избранное

- Keychain — Passcode

- Kingfisher — загрузка и кэш изображений

#🧭 Навигация

- Навигация вынесена в Routers, создание модулей через Builder

#🎛 UI

*UICollectionView (CompositionalLayout & FlowLayout)

*UITableView

*UITabBarController

*UIStackView

*NSLayoutConstraint

#📱 Скриншоты

![Main Screen](Screenshots/main_screen.png)
![Main Screen + Search](Screenshots/main_screen_search.png)

![Movie Page/1](Screenshots/movie_page1.png)
![Movie Page/2](Screenshots/movie_page2.png)
![Movie Page/3](Screenshots/movie_page2.png)

![TVSeries Page/1](Screenshots/series_page1.png)
![TVSeries Page/2](Screenshots/series_page2.png)
![TVSeries Page/3](Screenshots/series_page3.png)

![Actor Page/1](Screenshots/actor_filmography.png)
![Actor Page/2](Screenshots/actor_biography.png)

![Trailer Player](Screenshots/trailer_player.png)

![Favorites Page](Screenshots/favorite_page.png)

![Dynamic List Page: Genres, TopMovie, TopTVSeries, Upcoming](Screenshots/dynamic_list_page.png)

![Passcode Page](Screenshots/passcode_page.png)

![Setting Page](Screenshots/setting_page.png)

